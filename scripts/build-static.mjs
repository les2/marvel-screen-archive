...
const outputFiles = (directory) => readdirSync(directory,{withFileTypes:true}).flatMap((entry) => entry.isDirectory() ? outputFiles(resolve(directory,entry.name)) : [resolve(directory,entry.name)]);
const releaseHash = createHash('sha256');
for (const file of outputFiles(output).filter((file) => file !== swPath).sort()) releaseHash.update(relative(output,file)).update(readFileSync(file));
const release = releaseHash.digest('hex').slice(0,12);
const sw = readFileSync(swPath,'utf8').replace('__MAIN_ASSET__',`/${mainFile}`).replace('__STYLE_ASSET__',`/${stylesFile}`).replace('__BUILD_VERSION__',release);
