...
- A contextual 41-hour Countdown Protocol for *Avengers: Doomsday*, opened from its movie details or a compact latest-guide card, with private device-local checkmarks
...
- Semantic “who is this?” character summaries plus searchable powers, skills, affiliations, alignment, power status, and clearly separated derived appearance summaries
...
`scripts/expand-catalog.mjs` expands catalog-level coverage, generates labeled editorial descriptions, and derives release navigation. `scripts/enrich-catalog-data.mjs` consumes IMDb's `title.basics.tsv.gz` and `title.principals.tsv.gz`, resolves public Wikidata identifiers, expands characters, classifies appearances, and replaces search links with direct destinations when possible. `scripts/apply-character-profiles.mjs` then separates derived catalog summaries from semantic biographies and applies researched powers, skills, affiliations, and status labels. `scripts/derive-entities.mjs` materializes relation files. See [DATA_MODEL.md](DATA_MODEL.md) for entity and versioning decisions.
